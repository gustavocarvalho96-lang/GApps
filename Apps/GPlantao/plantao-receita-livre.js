function renderFreeGroups(body) {
  ...
  [
    ["dor", "Dor/Febre"],
    ["gastro", "Nausea/Gastro"],
    ["resp", "Alergia/Respiratorio"],
    ["antibiotics", "Antibioticos"],
    ["orientacoes", "Orientacoes/Sinais de Alarme"]
    ...
    if (state.openGroups[key]) {
      ...
      var source =
        key === "antibiotics"
          ? (window.antibioticOptions || []).map(function (o) {
        ...
        .forEach(function (item) {
          ...
          row.appendChild(
            dotButton(
              ...
              (state.editableText || "").indexOf(text) >= 0
            )
          );
        });
