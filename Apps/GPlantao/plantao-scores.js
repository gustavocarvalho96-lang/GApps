...
function heartResult() {
  ...
  var score =
    Number(s.history) +
    Number(s.ecg) +
    heartAgePoints(s.age) +
    Number(s.riskFactors) +
    Number(s.troponin);
...
function graceResult() {
  ...
  var risk =
    score <= 108
      ? "baixo (<1% intra-hospitalar)"
      : score <= 140
        ? "intermediario (1-3% intra-hospitalar)"
        : "alto (>3% intra-hospitalar)";
  return {
    score: score,
    risk: risk,
    text:
...
function optionField(scoreId, label, key, options) {
  ...
  return scoreField(
    label,
    selectInput(score[key], options, function (value) {
      score[key] = Number(value);
      render();
    })
  );
...
function smallNumberField(scoreId, label, key, placeholder) {
  ...
  return scoreField(
    label,
    numberInput(score[key], placeholder || "", function (value) {
      score[key] = value;
    })
  );
}
...
function scoreSuggestedConduct(id, result) {
  ...
  var map = {
    ...
    qsofa:
      summary.indexOf("risco aumentado") >= 0
    ...
    cchr:
      summary.indexOf("sem criterio") >= 0
    ...
    ciwa:
      summary.indexOf("grave") >= 0 || summary.indexOf("moderada") >= 0
    ...
    pesi:
      summary.indexOf("classe i") >= 0 || summary.indexOf("classe ii") >= 0
...
function renderSimpleScore(parent, config) {
  ...
  config.fields.forEach(function (field) {
    if (field.type === "number")
      grid.appendChild(smallNumberField(config.id, field.label, field.key, field.placeholder));
    if (field.type === "yesno")
      grid.appendChild(yesNoField(config.id, field.label, field.key, field.points));
    if (field.type === "option")
      grid.appendChild(optionField(config.id, field.label, field.key, field.options));
  });
  ...
  box.appendChild(
    textButton("Inserir " + config.short, "text-btn primary-btn", function () {
...
function renderScorePanel(body) {
  ...
  [
    ...
    tabs.appendChild(
      textButton(
        item[1],
        "text-btn" + (state.scores.tab === item[0] ? " active" : ""),
        function () {
  ...
  if (state.scores.tab === "cardio") {
    ...
    renderCardioScore(
      ...
      "heart",
      "HEART",
      heartData.score + "/10 - risco " + heartData.risk,
      function (heart) {
        var heartGrid = div("score-grid");
        heartGrid.appendChild(
          scoreField(
            "Historia",
            selectInput(
              state.scores.heart.history,
              [
                ["0", "0 - pouco/nada sugestiva"],
                ["1", "1 - moderada"],
                ["2", "2 - muito sugestiva"]
              ],
              function (value) {
        ...
        heartGrid.appendChild(
          scoreField(
            "ECG",
            selectInput(
              state.scores.heart.ecg,
              [
                ["0", "0 - normal"],
                ["1", "1 - inespecifico"],
                ["2", "2 - ST significativo"]
              ],
              function (value) {
        ...
        heartGrid.appendChild(
          scoreField(
            "Idade",
            numberInput(state.scores.heart.age, "anos", function (value) {
        ...
        heartGrid.appendChild(
          scoreField(
            "Fatores",
            selectInput(
              state.scores.heart.riskFactors,
              [
                ["0", "0 - nenhum"],
                ["1", "1 - 1 a 2"],
                ["2", "2 - >=3/DAC"]
        ...
        var heartConduct = scoreSuggestedConduct("heart", {
          summary: heartResultBox.textContent,
          text: heartData.text
        });
        if (heartConduct) {
        ...
        heart.appendChild(
          textButton("Inserir HEART", "text-btn primary-btn", function () {
    ...
    renderCardioScore(
      ...
      "grace",
      "GRACE",
      graceData.score + " pts - risco " + graceData.risk,
      function (grace) {
        var graceGrid = div("score-grid");
        graceGrid.appendChild(
          scoreField(
            "Idade",
            numberInput(state.scores.grace.age, "anos", function (value) {
        ...
        graceGrid.appendChild(
          scoreField(
            "FC",
            numberInput(state.scores.grace.heartRate, "bpm", function (value) {
        ...
        graceGrid.appendChild(
          scoreField(
            "PAS",
            numberInput(state.scores.grace.systolicBp, "mmHg", function (value) {
        ...
        graceGrid.appendChild(
          scoreField(
            "Creatinina",
            numberInput(state.scores.grace.creatinine, "mg/dL", function (value) {
        ...
        graceGrid.appendChild(
          scoreField(
            "Desvio ST",
            selectInput(
              state.scores.grace.stDeviation,
              [
                ["0", "Nao"],
                ["1", "Sim"]
              ],
        ...
        var graceConduct = scoreSuggestedConduct("grace", {
          summary: graceResultBox.textContent,
          text: graceData.text
        });
        if (graceConduct) {
        ...
        grace.appendChild(
          textButton("Inserir GRACE", "text-btn primary-btn", function () {
