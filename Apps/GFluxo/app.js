...
function getPersistableState() {
  return PERSISTED_FIELDS.reduce(
    (snapshot, field) => {
...
function restoreLocalState() {
  try {
    ...
    state.ocrStatus = saved.savedAt
      ? `Dados restaurados deste computador (${formatSavedTime(new Date(saved.savedAt))}).`
      : "Dados restaurados deste computador.";
  } catch {
...
function nameLooksSuspicious(name) {
  ...
  return /(\bTrack\b|\bOutros\b|\bClinica\b|\bMedica\b|\bOdontologia\b|\bPaciente\b|\bIdade\b|\bTempo\b|\bAtendimento\b|\bEspecialidade\b|\bSetor\b)/i.test(
    safe
  );
}
...
function parseOfficeConfig(text) {
  const base = Array.from({ length: 9 }, (_, idx) => ({
    id: String(idx + 1),
    name: "",
    active: false
  }));
...
function parsePatients(text) {
  return text
    ...
    .filter(Boolean)
    .map((line, index) => {
      const [name = "", age = "", onc = "", odonto = "", confidence = "100", fastTrack = "nao"] =
...
function distributePatients(patients, offices, startIndex = 0) {
  ...
  const assigned = activePatients.map((patient, idx) => ({
    ...patient,
    consultorio: offices[(startIndex + idx) % offices.length]
  }));
...
function extractCandidateFromLine(line) {
  ...
  const isDigitsOnly = (value) =>
    value.length > 0 && value.split("").every((ch) => ch >= "0" && ch <= "9");
  const hasLetters = (value) => value.split("").some((ch) => isLatinLetter(ch));
  const firstToken = tokens[0] || "";
  const lastToken = tokens[tokens.length - 1] || "";
  const looksLikeHospitalLine =
    tokens.length >= 2 &&
    ((isDigitsOnly(firstToken) && firstToken.length >= 3) ||
      (isDigitsOnly(lastToken) && lastToken.length <= 2 && tokens.filter(hasLetters).length >= 2));
  if (looksLikeHospitalLine) {
    const work = [...tokens];
    if (work.length && isDigitsOnly(work[0]) && work[0].length >= 3) work.shift();
    while (work.length && ["A", "V", "DE", "YC", "VV"].includes(work[0].toUpperCase()))
      work.shift();
    while (work.length && isDigitsOnly(work[work.length - 1]) && work[work.length - 1].length <= 2)
      work.pop();
    ...
    if (alphaTokens.length >= 2) {
      ...
      if (
        cleanedHospitalName &&
        cleanedHospitalName.length >= 6 &&
        !nameLooksSuspicious(cleanedHospitalName)
      )
        return {
  ...
  return {
    name: cleanedName,
    age,
    odonto: loose.includes("ODONTOLOGIA"),
    oncologico: loose.includes("ONCO"),
    fastTrack: loose.includes("FAST TRACK")
  };
}
function parseHospitalScreenshotText(ocrText) {
  ...
  const lines = ocrText
    ...
    .map(normalizeText)
    .filter(Boolean)
    .filter((line) => !line.toLowerCase().includes("hospital sao vicente"))
    .filter((line) => !line.toLowerCase().includes("novo hospital"));
  for (const line of lines) {
    const candidate = extractCandidateFromLine(line);
    if (!candidate || nameLooksSuspicious(candidate.name)) continue;
    patients.push({
      id: `${patients.length}-${candidate.name}`,
      name: candidate.name,
      age: candidate.age,
      oncologico: candidate.oncologico,
      odonto: candidate.odonto,
      fastTrack: candidate.fastTrack,
      confidence: 92
    });
  }
  ...
  for (const result of passResults) {
    for (const patient of result) {
      const existing = buckets.find(
        (bucket) =>
          bucket.age === patient.age && levenshteinSimilarity(bucket.names[0], patient.name) >= 0.84
      );
      if (existing) {
        existing.names.push(patient.name);
        existing.votes += 1;
        existing.odonto ||= patient.odonto;
        existing.oncologico ||= patient.oncologico;
        existing.fastTrack ||= patient.fastTrack;
      } else {
        buckets.push({
          age: patient.age,
          names: [patient.name],
          votes: 1,
          odonto: patient.odonto,
...
function chooseConsensusName(names) {
  ...
  for (const candidate of names) {
    const score = names.reduce(
      (sum, current) => sum + levenshteinSimilarity(candidate, current),
      0
    );
    if (score > bestScore) {
      bestScore = score;
...
function mergeMultiPassResults(passResults) {
  return groupConsensus(passResults)
    .map((bucket, index) => {
      const name = chooseConsensusName(bucket.names);
      const confidence = Math.min(
        99,
        70 + bucket.votes * 8 + Math.round(levenshteinSimilarity(name, bucket.names[0]) * 12)
      );
      return {
        id: `${index}-${name}`,
        name,
        age: bucket.age,
        odonto: bucket.odonto,
        oncologico: bucket.oncologico,
        fastTrack: bucket.fastTrack,
        confidence
      };
    })
...
function compareRemainingPatients(previousAssigned, currentDetected) {
  ...
  const namesMatch = (basePatient, currentPatient) => {
    ...
    const ageMatch =
      basePatient.age !== null &&
      currentPatient.age !== null &&
      basePatient.age === currentPatient.age;
    if (commonTokens.length >= 2 || (commonTokens.length >= 1 && ageMatch)) return true;
    if (
      baseTokens[0] &&
      baseTokens[0] === curTokens[0] &&
      baseTokens.at(-1) &&
      baseTokens.at(-1) === curTokens.at(-1)
    )
      return true;
    if (baseTokens.at(-1) && baseTokens.at(-1) === curTokens.at(-1) && ageMatch) return true;
    return similarity >= 0.78 || (similarity >= 0.68 && ageMatch);
  };
  const matched = previousAssigned
    .filter((patient) => currentDetected.some((current) => namesMatch(patient, current)))
    .map((patient) => ({
      id: patient.id,
      name: patient.name,
  ...
  return {
    matched,
    debug: {
      baselineNames: previousAssigned.map((p) => `${p.name}${p.age !== null ? ` (${p.age})` : ""}`),
      detectedNames: currentDetected.map((p) => `${p.name}${p.age !== null ? ` (${p.age})` : ""}`),
...
function getComputedData() {
  ...
  const consultorios = officeConfig
    .filter((office) => office.active && office.name.trim())
  ...
  const visiblePatients = patients.filter(
    (patient) => state.includeFastTrack || !patient.fastTrack
  );
  const distributed = distributePatients(visiblePatients, consultorios, state.rotationStartIndex);
  const previewColumns =
    distributed.assigned.length <= 8
      ? [distributed.assigned]
      : [
          distributed.assigned.slice(0, Math.ceil(distributed.assigned.length / 2)),
          distributed.assigned.slice(Math.ceil(distributed.assigned.length / 2))
        ];
  return {
    officeConfig,
    consultorios,
    ...
    patients,
    visiblePatients,
    assigned: distributed.assigned,
    odonto: distributed.odonto,
    previewColumns
  };
}
...
function renderLeftSections(data) {
  const sections = state.leftPanelOrder
    .map((sectionId) => {
      if (sectionId === "legend") {
        const offices = data.officeConfig
          .map(
            (office) => `
      ...
      if (sectionId === "upload") {
        return `<div class="section"><div class="section-head">${collapseButton(state.showUpload, "Enviar print", "", "toggleUpload")}${moveButtons("upload")}</div>${
          state.showUpload
            ? `
...
function renderPreview(data) {
  ...
  setHtml(
    ...
    data.legend.length
      ? data.legend
          .map(
            (office) =>
              `<div class="legend-item"><strong>${office.id}</strong><span>=</span><span>${escapeHtml(office.name)}</span></div>`
          )
          .join("")
      : `<div class="status error">Ative consultórios na lateral</div>`
  );
  ...
  setHtml(
    ...
    data.previewColumns
      .map(
        (columnPatients) =>
          `<div class="patient-list">${columnPatients.map((patient) => `<div class="patient-row"><div class="patient-name">${escapeHtml(patient.name.toUpperCase())}</div><div class="room">${escapeHtml(patient.consultorio)}</div></div>`).join("")}</div>`
      )
      .join("")
  );
  setHtml(
    "odontoPreview",
    data.odonto.length
      ? `<div class="odonto"><div class="odonto-title">Odontologia</div><div class="stack" style="gap:6px">${data.odonto.map((patient) => `<div class="odonto-row"><span>${escapeHtml(patient.name)}</span><span style="color:var(--muted)">${escapeHtml(patient.age ?? "")}</span></div>`).join("")}</div></div>`
      : ""
  );
  const confidenceCutoff = state.precisionMode ? 96 : 92;
  const flagged = [...data.assigned, ...data.odonto].filter(
    (patient) => patient.confidence < confidenceCutoff || nameLooksSuspicious(patient.name)
  );
...
function renderReadingSummary() {
  ...
  setHtml(
    "readingSummary",
    state.ocrLog
      ? `${collapseButton(state.showReadingSummary, "Resumo das leituras", "Abrir / fechar", "toggleReading")}${state.showReadingSummary ? `<textarea id="ocrLog" class="input" rows="7">${escapeHtml(state.ocrLog)}</textarea>` : ""}`
      : ""
  );
}
...
function attachDynamicListeners() {
  ...
  document.querySelectorAll("[data-office-toggle]").forEach((el) => {
    el.onclick = () => {
      const data = getComputedData();
      const next = data.officeConfig.map((item) =>
        item.id === el.dataset.officeToggle ? { ...item, active: !item.active } : item
      );
  ...
  document.querySelectorAll("[data-office-name]").forEach((el) => {
    el.oninput = () => {
      const data = getComputedData();
      const next = data.officeConfig.map((item) =>
        item.id === el.dataset.officeName
          ? { ...item, name: el.value, active: el.value.trim() ? true : item.active }
          : item
      );
      state.legendInput = serializeOfficeConfig(next);
  ...
  if (verificationFile)
    verificationFile.onchange = () =>
      processVerificationImageFile(verificationFile.files && verificationFile.files[0]);
  ...
  if (dropzone) {
    ...
    dropzone.ondrop = (event) => {
      event.preventDefault();
      dropzone.classList.remove("drag");
      processImageFile(event.dataTransfer.files && event.dataTransfer.files[0]);
    };
  }
}
...
function runAction(action) {
  ...
  if (action === "chooseMain") {
    state.pasteTarget = "main";
    $("mainFile")?.click();
    return;
  }
  if (action === "chooseVerification") {
    state.pasteTarget = "verification";
...
async function runOcrPass(imageSource, config, onStatus) {
  if (!window.Tesseract)
    throw new Error("Tesseract nao carregou. Reabra o app ou use entrada manual.");
  const result = await Tesseract.recognize(imageSource, "por+eng", {
    ...
    logger: (message) => {
      if (message.status === "recognizing text")
        onStatus(`${config.label}: ${Math.round((message.progress || 0) * 100)}%`);
    }
  });
...
async function extractPatientsFromFile(file, setStatus) {
  if (!file || !file.type.startsWith("image/"))
    throw new Error("O arquivo precisa ser uma imagem valida.");
  ...
  for (const pass of passes) {
    ...
    const patientsFromPass = await runOcrPass(source, pass, (message) =>
      setStatus(`Leitura ${message}`)
    );
    passResults.push(patientsFromPass);
    logLines.push(
      `${pass.label}: ${patientsFromPass.map((p) => `${p.name} (${p.age ?? "?"})`).join(" | ") || "sem pacientes"}`
    );
  }
  let extractedPatients = mergeMultiPassResults(passResults).filter(
    (patient) => !nameLooksSuspicious(patient.name)
  );
...
async function processImageFile(file) {
  if (!file || !file.type.startsWith("image/")) {
    state.ocrStatus = "O arquivo precisa ser uma imagem valida.";
    render();
    return;
  }
  state.ocrBusy = true;
  state.generatedImageDataUrl = "";
  state.passSummary = "";
  state.ocrStatus = "Preparando imagem...";
  render();
  try {
    const { dataUrl, extractedPatients, logLines, passesCount } = await extractPatientsFromFile(
      file,
      (value) => {
        state.ocrStatus = value;
        render();
      }
    );
    ...
    if (!extractedPatients.length) {
      ...
      state.ocrStatus =
        "Nao consegui montar pacientes com seguranca. Tente outra imagem ou ajuste manualmente.";
      return;
    }
    state.rawInput = extractedPatients
      .map(
        (patient) =>
          `${patient.name};${patient.age ?? ""};${patient.oncologico ? "sim" : "nao"};${patient.odonto ? "sim" : "nao"};${patient.confidence};${patient.fastTrack ? "sim" : "nao"}`
      )
      .join("\n");
...
async function processVerificationImageFile(file) {
  if (!state.verificationBase.length) {
    state.ocrStatus = "Primeiro ative a verificacao com uma base carregada.";
    render();
    return;
  }
  ...
  render();
  try {
    const { dataUrl, extractedPatients, logLines } = await extractPatientsFromFile(
      file,
      (value) => {
        state.ocrStatus = value;
        render();
    ...
    state.verificationDebug = {
      baselineNames: state.verificationBase.map(
        (p) => `${p.name}${p.age !== null ? ` (${p.age})` : ""}`
      ),
      detectedNames: extractedPatients.map(
        (p) => `${p.name}${p.age !== null ? ` (${p.age})` : ""}`
      ),
      matchedNames: verification.debug.matchedNames
    };
    state.ocrLog = logLines.join("\n\n");
    state.ocrStatus = verification.matched.length
      ? `${verification.matched.length} pacientes continuam na nova imagem.`
      : "Nenhum paciente da base foi encontrado na nova imagem.";
  } catch (error) {
...
$("closeAllBtn").onclick = () => {
  ...
  state.showExtractedPatients = false;
  state.showReadingSummary = false;
  scheduleLocalSave();
  render();
};
$("rotateBtn").onclick = () => {
  const data = getComputedData();
  state.rotationStartIndex = data.consultorios.length
    ? (state.rotationStartIndex + 1) % data.consultorios.length
    : 0;
  scheduleLocalSave();
  render();
};
$("fastTrackBtn").onclick = () => {
  state.includeFastTrack = !state.includeFastTrack;
  scheduleLocalSave();
  render();
};
...
$("verificationBtn").onclick = () => {
  ...
  if (state.verificationMode) {
    state.verificationBase = data.assigned.length
      ? data.assigned.map((patient) => ({ ...patient }))
      : [];
    ...
    state.ocrStatus = state.verificationBase.length
      ? `Verificacao ligada. Base congelada com ${state.verificationBase.length} pacientes.`
      : "Carregue pacientes antes de ativar a verificacao.";
  } else {
    state.verificationBase = [];
    state.pasteTarget = "main";
    state.verificationDebug = { baselineNames: [], detectedNames: [], matchedNames: [] };
