...
function transcribeLabs(rawText) {
  ...
  var bt = examResult(["BILIRRUBINA TOTAL", "BILIRRUBINAS TOTAL", "BILLIRUBINA TOTAL", "BILLIRRUBINAS TOTAL"]);
  ...
  if (coag) {
    var tp = coagValue(coag, ["Tempo de Protrombina:", "Tempo de Protrombina", "TP:"], 2);
    var inr = coagValue(coag, ["INR:", "INR"], 1);
    var ttpa = coagValue(coag, ["TTPA:", "TTPA", "Tempo de Tromboplastina Parcial Ativada"], 2);
    push("Coagulograma (TP = " + (tp || "") + "; INR = " + (inr || "") + "; TTPA = " + (ttpa || "") + ")");
  }
